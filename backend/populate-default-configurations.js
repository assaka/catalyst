...
async function populateDefaultConfigurations() {
  try {
    ...
    // Create proper default configuration (define it first so we can use it in update)
    const defaultConfig = {
        page_name: 'Cart',
        slots: {
          // Header content
    ...
    if (existingDraft.length > 0) {
      ...
      // Update the existing configuration instead of creating new one
      ...
      console.log(`✅ Updated existing configuration with complete slot content`);
      console.log(`🎉 Configuration updated successfully!`);
      return;
    }

    console.log(`📋 Creating draft configuration for store ${storeId}`);
