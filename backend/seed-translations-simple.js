...
async function seedTranslations() {
  try {
    ...
    console.log(`📝 Inserting ${defaultTranslations.length} translations...`);

    // Insert one by one to avoid bulkInsert issues
    let inserted = 0;
    for (const t of defaultTranslations) {
      await sequelize.query(`
        INSERT INTO translations (id, key, language_code, value, category, created_at, updated_at)
        VALUES (gen_random_uuid(), :key, 'en', :value, :category, NOW(), NOW())
      ...
      inserted++;
      if (inserted % 10 === 0) {
        process.stdout.write(`\r  Progress: ${inserted}/${defaultTranslations.length}`);
      }
    }
