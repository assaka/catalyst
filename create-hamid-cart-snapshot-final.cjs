...
(async () => {
  try {
    console.log('🔧 Creating Hamid Cart snapshot with correct change_type...');
    ...
    `, {
      replacements: {
        ...
        change_type: 'manual_edit', // Using valid constraint value
    ...
    console.log('   Change Type: manual_edit');
    ...
    if (patches.length > 0) {
      ...
      console.log('     Snapshot Number: ' + (latestPatch.snapshot_number || 'unknown'));
      console.log('     Change Summary: ' + latestPatch.change_summary);
      console.log('     Has diffHunks: ' + !!latestPatch.diffHunks);
      
      if (latestPatch.id === snapshotId) {
        console.log('     ✅ Service is returning the NEW snapshot!');
      } else {
        console.log('     ⚠️ Service is still returning old snapshot: ' + latestPatch.id);
      }
      
      if (latestPatch.diffHunks && latestPatch.diffHunks.length > 0) {
        console.log('     DiffHunks: ' + latestPatch.diffHunks.length + ' hunk(s)');
        ...
        if (foundHamidCart) {
          console.log('\n🎉 COMPLETE SUCCESS!');
          console.log('   ✅ Hamid Cart snapshot created and active');
          console.log('   ✅ Diff service returns Hamid Cart patch');
          console.log('   ✅ Diff tab should now show "My Cart" → "Hamid Cart" change');
        ...
        console.log('     ❌ No diff hunks in latest patch');
      }
    } else {
