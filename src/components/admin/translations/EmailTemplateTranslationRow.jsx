...
import { ChevronDown, ChevronRight, Globe, Wand2, Mail } from 'lucide-react';
...
export default function EmailTemplateTranslationRow({ template, onUpdate, selectedLanguages, onFlashMessage }) {
  ...
  const getTranslationStatus = () => {
    const translatedCount = filteredLanguages.filter(lang => {
      ...
      return translation && translation.subject && translation.subject.trim().length > 0;
    }).length;
  ...
  // Fields to translate based on content type
  const getFields = () => {
    const fields = [{ key: 'subject', label: 'Subject', multiline: false }];

    if (template.content_type === 'template' || template.content_type === 'both') {
      fields.push({ key: 'template_content', label: 'Template Content', multiline: true });
    }

    if (template.content_type === 'html' || template.content_type === 'both') {
      fields.push({ key: 'html_content', label: 'HTML Content', multiline: true });
    }
  ...
  const handleSave = async () => {
    try {
      ...
      toast.success('Email template translations updated successfully');
      if (onFlashMessage) onFlashMessage('Email template translations updated successfully', 'success');
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500 truncate">
              {translations.en?.subject || 'No subject'}
            </p>
          </div>
        </button>
