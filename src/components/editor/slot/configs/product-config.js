...
export const productConfig = {
  ...
  slots: {
    ...
    product_labels: {
      id: 'product_labels',
      type: 'text',
      content: '{{#if productLabels}}{{#each productLabels}}<div class="absolute {{#if (eq this.position "top-left")}}top-2 left-2{{else}}{{#if (eq this.position "bottom-left")}}bottom-2 left-2{{else}}{{#if (eq this.position "bottom-right")}}bottom-2 right-2{{else}}{{#if (eq this.position "center")}}top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2{{else}}top-2 right-2{{/if}}{{/if}}{{/if}}{{/if}} text-xs font-semibold px-2 py-1 rounded-md z-10" style="background-color: {{this.background_color}}; color: {{this.text_color}};">{{this.text}}</div>{{/each}}{{/if}}',
