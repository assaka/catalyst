...
export const productConfig = {
  ...
  slots: {
    ...
    thumbnail_gallery: {
      ...
      content: '',
      className: 'thumbnail-gallery {{#if (eq settings.product_gallery_layout "vertical")}}flex flex-col space-y-2 w-20{{else}}flex space-x-2 overflow-x-auto{{/if}} {{#if (eq settings.vertical_gallery_position "right")}}order-last pl-4{{/if}}{{#if (eq settings.vertical_gallery_position "left")}}order-first pr-4{{/if}}{{#if (eq settings.product_gallery_layout "horizontal")}}order-2 mt-4{{/if}}',
      styles: {},
      parentId: 'gallery_container',
      position: { col: 1, row: 1 },
      layout: 'flex',
