...
// ProductActionsSlot Component - Add to cart, quantity, wishlist
export function ProductActionsSlot({ productContext, content }) {
  const { product, settings } = productContext;
  ...
  const canAddToCart = isInStock && quantity > 0 && (product.infinite_stock || product.stock_quantity >= quantity);

  return (
    <div className="product-actions border-t pt-6">
      {content ? (
        <div dangerouslySetInnerHTML={{ __html: content }} />
      ) : (
        <div className="space-y-4">
          ...
          {!settings?.hide_quantity_selector && (
            <div className="flex items-center space-x-2">
              <label htmlFor="quantity" className="font-medium text-sm">Qty:</label>
