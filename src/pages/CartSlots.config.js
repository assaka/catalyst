/**
 * CartSlots.config.js - SCHEMA-BASED CONFIGURATION
 * Type-safe, validated slot configuration for cart page
 * This file defines the structure without importing React components
 */

import { SlotConfigBuilder } from '@/core/slot-system/SlotSchema.js';

// =============================================================================
// 🏗️ SCHEMA-BASED CART SLOT CONFIGURATION
// =============================================================================

export const CART_SLOTS_CONFIG = new SlotConfigBuilder('Cart')
  
  // Page structure slots
  .addSlot({
    id: 'cart-page-container',
    type: 'container',
    component: 'SlotCartPageContainer', // String reference to avoid import issues
    name: 'Page Container',
...
export const CART_SLOT_DEFINITIONS = CART_SLOTS_CONFIG.slotDefinitions;
export const CART_SLOT_ORDER = CART_SLOTS_CONFIG.slotOrder;  
export const CART_LAYOUT_PRESETS = CART_SLOTS_CONFIG.layoutPresets;

// Export utilities
export function getSlotById(id) {
  return CART_SLOTS_CONFIG.slotDefinitions[id];
}

export function getSlotsByType(type) {
  return Object.values(CART_SLOTS_CONFIG.slotDefinitions)
    .filter(slot => slot.type === type);
}

export function getSlotsByCategory(category) {
  return Object.values(CART_SLOTS_CONFIG.slotDefinitions)
    .filter(slot => slot.metadata?.category === category);
}

export function validateSlotOrder(newOrder) {
  const { SlotValidator } = require('@/core/slot-system/SlotSchema.js');
  return SlotValidator.validateSlotOrder(newOrder, CART_SLOTS_CONFIG.slotDefinitions);
}

// Log validation result
console.log('✅ Schema-based Cart slot configuration loaded with', 
  Object.keys(CART_SLOT_DEFINITIONS).length, 'slots');

export default CART_SLOTS_CONFIG;