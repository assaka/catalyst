...
class SlotConfigurationService {
  ...
  async getDraftConfiguration(storeId, pageType = 'cart') {
    try {
      const response = await axios.get(`${API_BASE}/draft/${storeId}/${pageType}`);
  ...
  async getPublishedConfiguration(storeId, pageType = 'cart') {
    try {
      const response = await axios.get(`${API_BASE}/published/${storeId}/${pageType}`);
      return response.data;
    } catch (error) {
  ...
  async updateDraftConfiguration(configId, configuration) {
    try {
      const response = await axios.put(`${API_BASE}/draft/${configId}`, {
        configuration
      });
      return response.data;
    } catch (error) {
  ...
  async publishDraft(configId) {
    try {
      const response = await axios.post(`${API_BASE}/publish/${configId}`);
      return response.data;
    } catch (error) {
  ...
  async getVersionHistory(storeId, pageType = 'cart', limit = 20) {
    try {
      const response = await axios.get(`${API_BASE}/history/${storeId}/${pageType}`, {
        params: { limit }
      });
      return response.data;
    } catch (error) {
  ...
  async revertToVersion(versionId) {
    try {
      const response = await axios.post(`${API_BASE}/revert/${versionId}`);
      return response.data;
    } catch (error) {
  ...
  async deleteDraft(configId) {
    try {
      const response = await axios.delete(`${API_BASE}/draft/${configId}`);
      return response.data;
    } catch (error) {
